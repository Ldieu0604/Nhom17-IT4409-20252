...
export function RecentDocuments() {
  ...
  return <section className="py-8">
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold sm:text-2xl">Tài liệu gần đây</h2><p className="text-sm text-muted-foreground">Các tài liệu bạn đã truy cập gần đây</p></div><Link href="/dashboard" className="inline-flex w-full items-center justify-center rounded-md border px-3 py-2 text-sm hover:border-primary sm:w-auto">Xem tất cả</Link></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{documents.map((document) => <Link href={`/documents/${document.id}`} key={document.id} className="rounded-xl border bg-white p-4 transition hover:border-primary/50 hover:shadow-sm sm:p-5"><FileText className="h-5 w-5 text-primary" /><h3 className="mt-3 truncate font-semibold">{document.title}</h3><p className="mt-3 flex items-center gap-1 text-xs text-muted-foreground"><Clock className="h-3.5 w-3.5" />{formatDocumentDate(document.openedAt || document.updatedAt)}</p></Link>)}</div>
