...
import {
  ...
  Pencil, Search, Trash2, Users, X,
} from "lucide-react"
import { createDashboardDocument, deleteDashboardDocument, renameDashboardDocument } from "@/services/document.service"
import {
  addWorkspaceMember, createWorkspace, createWorkspaceTask, deleteWorkspace, deleteWorkspaceMember,
  deleteWorkspaceTask, getWorkspace, listWorkspaces, updateWorkspaceMember, updateWorkspaceTask,
  type Workspace, type WorkspaceDocument, type WorkspaceInvitation, type WorkspaceMember, type WorkspaceTask,
...
type TaskSortKey = "dueDate" | "priority"
...
function TaskActionMenu({ workspace, task, reload }: { workspace: Workspace; task: WorkspaceTask; reload: () => void }) {
  const [editing, setEditing] = useState(false); const [confirmingDelete, setConfirmingDelete] = useState(false); const [saving, setSaving] = useState(false); const [deleting, setDeleting] = useState(false)
  const [title, setTitle] = useState(task.title); const [assigneeId, setAssigneeId] = useState(task.assigneeId || ""); const [documentId, setDocumentId] = useState(task.documentId || ""); const [status, setStatus] = useState<WorkspaceTask["status"]>(task.status); const [priority, setPriority] = useState<WorkspaceTask["priority"]>(task.priority); const [dueDate, setDueDate] = useState(dateInputValue(task.dueDate))
  function openEditor() { setTitle(task.title); setAssigneeId(task.assigneeId || ""); setDocumentId(task.documentId || ""); setStatus(task.status); setPriority(task.priority); setDueDate(dateInputValue(task.dueDate)); setEditing(true) }
  async function submit(event: React.FormEvent) { event.preventDefault(); if (!title.trim()) return; setSaving(true); try { await updateWorkspaceTask(workspace.id, task.id, { title: title.trim(), assigneeId: assigneeId || null, documentId: documentId || null, status, priority, dueDate: dueDate ? new Date(`${dueDate}T00:00:00`).toISOString() : null }); setEditing(false); reload() } finally { setSaving(false) } }
  async function removeTask() { setDeleting(true); try { await deleteWorkspaceTask(workspace.id, task.id); setConfirmingDelete(false); reload() } finally { setDeleting(false) } }
  return <>
    <DropdownMenu>
      <DropdownMenuTrigger asChild><button type="button" title="Tùy chọn công việc" className="rounded-full p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><MoreHorizontal className="h-4 w-4" /></button></DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-44"><DropdownMenuItem onSelect={openEditor}><Pencil className="h-4 w-4" />Chỉnh sửa</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={() => setConfirmingDelete(true)}><Trash2 className="h-4 w-4" />Xóa</DropdownMenuItem></DropdownMenuContent>
    </DropdownMenu>
    {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4"><form onSubmit={submit} className="relative grid w-full max-w-xl gap-3 rounded-xl border bg-white p-5 shadow-xl sm:grid-cols-2"><button type="button" onClick={() => setEditing(false)} className="absolute right-4 top-4"><X className="h-4 w-4" /></button><h3 className="sm:col-span-2 font-semibold">Chỉnh sửa công việc</h3><input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Tên công việc" className="rounded-md border px-3 py-2 text-sm sm:col-span-2" /><select value={assigneeId} onChange={(e) => setAssigneeId(e.target.value)} className="rounded-md border px-2 py-2 text-sm"><option value="">Chưa giao</option>{workspace.members.map((member) => <option key={member.id} value={member.user.id}>{member.user.displayName}</option>)}</select><select value={documentId} onChange={(e) => setDocumentId(e.target.value)} className="rounded-md border px-2 py-2 text-sm"><option value="">Không gắn tài liệu</option>{workspace.documents?.map((document) => <option key={document.id} value={document.id}>{document.title}</option>)}</select><select value={status} onChange={(e) => setStatus(e.target.value as WorkspaceTask["status"])} className="rounded-md border px-2 py-2 text-sm"><option value="todo">Chưa làm</option><option value="in_progress">Đang làm</option><option value="done">Hoàn thành</option></select><select value={priority} onChange={(e) => setPriority(e.target.value as WorkspaceTask["priority"])} className="rounded-md border px-2 py-2 text-sm"><option value="low">Thấp</option><option value="medium">Trung bình</option><option value="high">Cao</option></select><input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="rounded-md border px-2 py-2 text-sm" /><div className="flex justify-end gap-2 sm:col-span-2"><button type="button" onClick={() => setEditing(false)} className="rounded-md px-3 py-2 text-sm text-slate-500">Hủy</button><button disabled={saving} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white">{saving ? "Đang lưu..." : "Lưu"}</button></div></form></div>}
    <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xóa công việc?</AlertDialogTitle><AlertDialogDescription>Công việc &quot;{task.title}&quot; sẽ bị xóa khỏi workspace.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={deleting}>Hủy</AlertDialogCancel><AlertDialogAction disabled={deleting} onClick={() => void removeTask()} className="bg-red-600 hover:bg-red-700">{deleting ? "Đang xóa..." : "Xóa"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>
}
...
function TodoList({ workspace, tasks, reload }: { workspace: Workspace; tasks: WorkspaceTask[]; reload: () => void }) {
  ...
  return <Card className="p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h2 className="font-semibold">To-do List</h2><div className="flex flex-wrap gap-2"><select value={assignee} onChange={(e) => setAssignee(e.target.value)} className="rounded-md border px-2 py-1.5 text-xs"><option value="">Mọi người phụ trách</option>{workspace.members.map((member) => <option key={member.id} value={member.user.id}>{member.user.displayName}</option>)}</select><select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-md border px-2 py-1.5 text-xs"><option value="">Mọi trạng thái</option><option value="todo">Chưa làm</option><option value="in_progress">Đang làm</option><option value="done">Hoàn thành</option></select><select value={deadline} onChange={(e) => setDeadline(e.target.value)} className="rounded-md border px-2 py-1.5 text-xs"><option value="">Mọi deadline</option><option value="3">Trong 3 ngày</option><option value="7">Trong 7 ngày</option></select></div></div>{visibleTasks.map((task) => <div key={task.id} className="group flex items-center gap-3 border-t py-3 text-sm"><input type="checkbox" checked={task.completed} onChange={async () => { await updateWorkspaceTask(workspace.id, task.id, { completed: !task.completed }); reload() }} className="h-4 w-4 accent-primary" /><span className={`min-w-0 flex-1 truncate ${task.completed ? "text-slate-400 line-through" : "font-medium"}`}>{task.title}</span><span className="hidden text-xs text-slate-500 sm:block">{task.assignee?.displayName || "Chưa giao"}</span><StatusPill status={task.status} /><span className="hidden text-xs text-slate-500 md:block">{dateLabel(task.dueDate)}</span>{task.document && <Link href={`/documents/${task.document.id}`} className="hidden text-xs text-primary hover:underline lg:block">{task.document.title}</Link>}<span className="opacity-0 transition group-hover:opacity-100 focus-within:opacity-100"><TaskActionMenu workspace={workspace} task={task} reload={reload} /></span></div>)}{visibleTasks.length === 0 && <p className="py-6 text-sm text-slate-500">Không có công việc phù hợp.</p>}</Card>
}
...
function AssignmentTable({ workspace, tasks, reload, customColumns, onAddTask, onAddColumn }: { workspace: Workspace; tasks: WorkspaceTask[]; reload: () => void; customColumns: CustomColumn[]; onAddTask: () => void; onAddColumn: () => void }) {
  const [query, setQuery] = useState(""); const [assignee, setAssignee] = useState(""); const [status, setStatus] = useState(""); const [completion, setCompletion] = useState(""); const [sortKey, setSortKey] = useState<TaskSortKey>("dueDate"); const [sortDirection, setSortDirection] = useState<SortDirection>("asc")
  const activeFilterCount = [assignee, status, completion].filter(Boolean).length
  const priorityRank = { low: 1, medium: 2, high: 3 }
  const visibleTasks = [...tasks].filter((task) => {
    const keyword = query.trim().toLowerCase()
    return (!keyword || task.title.toLowerCase().includes(keyword)) && (!assignee || task.assigneeId === assignee) && (!status || task.status === status) && (!completion || (completion === "done" ? task.completed : !task.completed))
  }).sort((first, second) => {
    let result = 0
    if (sortKey === "dueDate") {
      ...
      else result = new Date(first.dueDate).getTime() - new Date(second.dueDate).getTime()
    } else {
  ...
  function clearFilters() { setQuery(""); setAssignee(""); setStatus(""); setCompletion("") }
  return <Card className="overflow-hidden"><div className="flex flex-wrap items-center gap-2 border-b p-3"><button onClick={onAddTask} className="rounded-md border px-3 py-1.5 text-xs font-medium hover:bg-slate-50"><Plus className="mr-1 inline h-3.5 w-3.5" />Hàng mới</button><button onClick={onAddColumn} className="rounded-md border px-3 py-1.5 text-xs font-medium hover:bg-slate-50"><Plus className="mr-1 inline h-3.5 w-3.5" />Cột mới</button><label className="ml-1 flex min-w-[220px] flex-1 items-center gap-2 rounded-md border px-3 py-1.5 text-xs text-slate-400"><Search className="h-3.5 w-3.5" /><input value={query} onChange={(e) => setQuery(e.target.value)} className="w-full bg-transparent outline-none" placeholder="Tìm theo tên công việc..." /></label><DropdownMenu><DropdownMenuTrigger asChild><button type="button" className={`rounded-md px-2 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 ${activeFilterCount ? "bg-emerald-50 text-primary" : ""}`}><Filter className="mr-1 inline h-3.5 w-3.5" />Bộ lọc{activeFilterCount ? ` (${activeFilterCount})` : ""}</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-64 p-3" onClick={(event) => event.stopPropagation()}><DropdownMenuLabel className="px-0 pt-0 text-xs text-slate-500">Lọc công việc</DropdownMenuLabel><label className="mt-2 block text-[11px] font-medium text-slate-500">Người phụ trách</label><select value={assignee} onChange={(e) => setAssignee(e.target.value)} className="mt-1 w-full rounded-md border bg-white px-2 py-1.5 text-xs"><option value="">Tất cả user</option>{workspace.members.map((member) => <option key={member.id} value={member.user.id}>{member.user.displayName}</option>)}</select><label className="mt-3 block text-[11px] font-medium text-slate-500">Trạng thái</label><select value={status} onChange={(e) => setStatus(e.target.value)} className="mt-1 w-full rounded-md border bg-white px-2 py-1.5 text-xs"><option value="">Tất cả trạng thái</option><option value="todo">Chưa làm</option><option value="in_progress">Đang làm</option><option value="done">Hoàn thành</option></select><label className="mt-3 block text-[11px] font-medium text-slate-500">Hoàn thành</label><select value={completion} onChange={(e) => setCompletion(e.target.value)} className="mt-1 w-full rounded-md border bg-white px-2 py-1.5 text-xs"><option value="">Tất cả</option><option value="done">Đã hoàn thành</option><option value="open">Chưa hoàn thành</option></select><button type="button" onClick={clearFilters} className="mt-3 w-full rounded-md border px-2 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">Xóa bộ lọc</button></DropdownMenuContent></DropdownMenu><DropdownMenu><DropdownMenuTrigger asChild><button type="button" className="rounded-md px-2 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50"><ArrowUpDown className="mr-1 inline h-3.5 w-3.5" />Sắp xếp</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-60 p-3" onClick={(event) => event.stopPropagation()}><DropdownMenuLabel className="px-0 pt-0 text-xs text-slate-500">Sắp xếp theo</DropdownMenuLabel><select value={sortKey} onChange={(e) => setSortKey(e.target.value as TaskSortKey)} className="mt-2 w-full rounded-md border bg-white px-2 py-1.5 text-xs"><option value="dueDate">Hạn</option><option value="priority">Ưu tiên</option></select><select value={sortDirection} onChange={(e) => setSortDirection(e.target.value as SortDirection)} className="mt-2 w-full rounded-md border bg-white px-2 py-1.5 text-xs"><option value="asc">{sortKey === "dueDate" ? "Gần hạn trước" : "Ưu tiên thấp trước"}</option><option value="desc">{sortKey === "dueDate" ? "Xa hạn trước" : "Ưu tiên cao trước"}</option></select></DropdownMenuContent></DropdownMenu></div>
    <div className="overflow-x-auto"><table className="w-full min-w-[900px] border-collapse text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr><th className="w-10 border-r px-3 py-2 text-center">#</th><th className="border-r px-3 py-2 font-medium">Aa&nbsp; Tên công việc</th><th className="border-r px-3 py-2 font-medium"><Users className="mr-1 inline h-3.5 w-3.5" />Người phụ trách</th><th className="border-r px-3 py-2 font-medium">Trạng thái</th><th className="border-r px-3 py-2 font-medium">Ưu tiên</th><th className="border-r px-3 py-2 font-medium"><CalendarDays className="mr-1 inline h-3.5 w-3.5" />Hạn</th>{customColumns.map((column) => <th key={column.id} className="border-r px-3 py-2 font-medium">{column.name}</th>)}<th className="border-r px-3 py-2 font-medium">Hoàn thành</th><th className="w-10 px-3 py-2" /></tr></thead><tbody>{visibleTasks.map((task, index) => <tr key={task.id} className="group border-t hover:bg-slate-50"><td className="border-r px-3 py-2 text-center text-slate-400">{index + 1}</td><td className="border-r px-3 py-2 font-medium">{task.title}</td><td className="border-r px-3 py-2"><span className="flex items-center gap-2"><Avatar name={task.assignee?.displayName || "?"} avatar={task.assignee?.avatar} size="sm" />{task.assignee?.displayName || "Chưa giao"}</span></td><td className="border-r px-3 py-2"><StatusPill status={task.status} /></td><td className="border-r px-3 py-2"><PriorityPill priority={task.priority} /></td><td className="border-r px-3 py-2">{dateLabel(task.dueDate)}</td>{customColumns.map((column) => <td key={column.id} className="border-r px-3 py-2 text-slate-400">-</td>)}<td className="border-r px-3 py-2 text-center"><input type="checkbox" checked={task.completed} onChange={async () => { await updateWorkspaceTask(workspace.id, task.id, { completed: !task.completed }); reload() }} className="h-4 w-4 accent-primary" /></td><td className="px-3 py-2"><span className="opacity-0 transition group-hover:opacity-100 focus-within:opacity-100"><TaskActionMenu workspace={workspace} task={task} reload={reload} /></span></td></tr>)}</tbody></table></div>
  </Card>
}
...
function WorkspaceDocumentActionMenu({ document, reload }: { document: WorkspaceDocument; reload: () => void }) {
  const [editing, setEditing] = useState(false); const [confirmingDelete, setConfirmingDelete] = useState(false); const [title, setTitle] = useState(document.title); const [saving, setSaving] = useState(false); const [deleting, setDeleting] = useState(false)
  function openEditor() { setTitle(document.title); setEditing(true) }
  async function submit(event: React.FormEvent) { event.preventDefault(); if (!title.trim()) return; setSaving(true); try { await renameDashboardDocument(document.id, title.trim()); setEditing(false); reload() } finally { setSaving(false) } }
  async function removeDocument() { setDeleting(true); try { await deleteDashboardDocument(document.id); setConfirmingDelete(false); reload() } finally { setDeleting(false) } }
  return <>
    <DropdownMenu>
      <DropdownMenuTrigger asChild><button type="button" title="Tùy chọn tài liệu" onClick={(event) => event.preventDefault()} className="rounded-full p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><MoreHorizontal className="h-4 w-4" /></button></DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-44"><DropdownMenuItem onSelect={openEditor}><Pencil className="h-4 w-4" />Chỉnh sửa</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={() => setConfirmingDelete(true)}><Trash2 className="h-4 w-4" />Xóa</DropdownMenuItem></DropdownMenuContent>
    </DropdownMenu>
    {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4"><form onSubmit={submit} className="relative w-full max-w-sm rounded-xl border bg-white p-5 shadow-xl"><button type="button" onClick={() => setEditing(false)} className="absolute right-4 top-4"><X className="h-4 w-4" /></button><h3 className="font-semibold">Chỉnh sửa tài liệu</h3><input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Tên tài liệu" className="mt-4 w-full rounded-md border px-3 py-2 text-sm" /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditing(false)} className="rounded-md px-3 py-2 text-sm text-slate-500">Hủy</button><button disabled={saving || !title.trim()} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white">{saving ? "Đang lưu..." : "Lưu"}</button></div></form></div>}
    <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xóa tài liệu?</AlertDialogTitle><AlertDialogDescription>Tài liệu &quot;{document.title}&quot; sẽ bị xóa vĩnh viễn.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={deleting}>Hủy</AlertDialogCancel><AlertDialogAction disabled={deleting} onClick={() => void removeDocument()} className="bg-red-600 hover:bg-red-700">{deleting ? "Đang xóa..." : "Xóa"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>
}

function WorkspaceDocuments({ workspace, createDoc, reload }: { workspace: Workspace; createDoc: (template: WorkspaceDocument["template"]) => void; reload: () => void }) {
...
function MemberActionMenu({ workspace, member, reload }: { workspace: Workspace; member: WorkspaceMember; reload: () => void }) {
  const [editing, setEditing] = useState(false); const [confirmingDelete, setConfirmingDelete] = useState(false); const [role, setRole] = useState<WorkspaceMember["role"]>(member.role); const [saving, setSaving] = useState(false); const [deleting, setDeleting] = useState(false)
  const isOwner = member.role === "owner"
  function openEditor() { setRole(member.role); setEditing(true) }
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); try { await updateWorkspaceMember(workspace.id, member.id, { role }); setEditing(false); reload() } finally { setSaving(false) } }
  async function removeMember() { setDeleting(true); try { await deleteWorkspaceMember(workspace.id, member.id); setConfirmingDelete(false); reload() } finally { setDeleting(false) } }
  return <>
    <DropdownMenu>
      <DropdownMenuTrigger asChild><button type="button" title="Tùy chọn thành viên" className="rounded-full p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><MoreHorizontal className="h-4 w-4" /></button></DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-44"><DropdownMenuItem onSelect={openEditor}><Pencil className="h-4 w-4" />Chỉnh sửa</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem disabled={isOwner} variant="destructive" onSelect={() => setConfirmingDelete(true)}><Trash2 className="h-4 w-4" />Xóa</DropdownMenuItem></DropdownMenuContent>
    </DropdownMenu>
    {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4"><form onSubmit={submit} className="relative w-full max-w-sm rounded-xl border bg-white p-5 shadow-xl"><button type="button" onClick={() => setEditing(false)} className="absolute right-4 top-4"><X className="h-4 w-4" /></button><h3 className="font-semibold">Chỉnh sửa thành viên</h3><p className="mt-1 text-sm text-slate-500">{member.user.displayName}</p><select value={role} onChange={(e) => setRole(e.target.value as WorkspaceMember["role"])} className="mt-4 w-full rounded-md border px-3 py-2 text-sm"><option value="member">Member</option><option value="owner">Owner</option></select><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditing(false)} className="rounded-md px-3 py-2 text-sm text-slate-500">Hủy</button><button disabled={saving} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white">{saving ? "Đang lưu..." : "Lưu"}</button></div></form></div>}
    <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xóa thành viên?</AlertDialogTitle><AlertDialogDescription>{member.user.displayName} sẽ bị xóa khỏi workspace này.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={deleting}>Hủy</AlertDialogCancel><AlertDialogAction disabled={deleting} onClick={() => void removeMember()} className="bg-red-600 hover:bg-red-700">{deleting ? "Đang xóa..." : "Xóa"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>
}
...
function WorkspaceMembers({ workspace, tasks, reload }: { workspace: Workspace; tasks: WorkspaceTask[]; reload: () => void }) {
  ...
  async function invite(event: React.FormEvent) {
    ...
    try {
      ...
      setInviteMessage(invitation.existingAccount ? "Đã gửi lời mời và thông báo trong app." : "Đã gửi email mời. Người nhận có thể đăng ký rồi chấp nhận lời mời.")
      reload()
    } catch (error) {
  ...
  return <div><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">Thành viên</h2><p className="text-sm text-slate-500">Quản lý thành viên, lời mời và tải công việc trong nhóm.</p></div>{workspace.role === "owner" && <form onSubmit={invite} className="flex flex-col items-start gap-2 sm:items-end"><div className="flex gap-2"><input type="email" value={email} onChange={(e) => { setEmail(e.target.value); if (inviteStatus !== "sending") setInviteStatus("idle") }} placeholder="Email thành viên" className="rounded-md border bg-white px-3 py-2 text-sm" /><button disabled={inviteStatus === "sending"} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60">{inviteStatus === "sending" ? "Đang gửi..." : "Mời thành viên"}</button></div>{inviteMessage && <p className={`max-w-sm text-xs ${inviteStatus === "error" ? "text-red-600" : "text-emerald-700"}`}>{inviteMessage}</p>}</form>}</div>
    <Card className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-4 py-3 font-medium">Thành viên</th><th className="px-4 py-3 font-medium">Vai trò</th><th className="px-4 py-3 font-medium">Tổng task</th><th className="px-4 py-3 font-medium">Đã hoàn thành</th><th className="px-4 py-3 font-medium">Đang làm</th><th className="px-4 py-3 font-medium">Deadline gần nhất</th><th /></tr></thead><tbody>{workspace.members.map((member) => { const assigned = tasks.filter((task) => task.assigneeId === member.user.id); const next = assigned.filter((task) => !task.completed && task.dueDate).sort((a, b) => new Date(a.dueDate!).getTime() - new Date(b.dueDate!).getTime())[0]; return <tr key={member.id} className="group border-t hover:bg-slate-50"><td className="px-4 py-3"><span className="flex items-center gap-2"><Avatar name={member.user.displayName} avatar={member.user.avatar} /><span><strong className="block text-sm">{member.user.displayName}</strong><span className="text-xs text-slate-500">{member.user.email}</span></span></span></td><td className="px-4 py-3 capitalize">{member.role}</td><td className="px-4 py-3">{assigned.length}</td><td className="px-4 py-3">{assigned.filter((task) => task.completed).length}</td><td className="px-4 py-3">{assigned.filter((task) => task.status === "in_progress").length}</td><td className="px-4 py-3">{next ? dateLabel(next.dueDate) : "-"}</td><td className="px-4 py-3"><span className="opacity-0 transition group-hover:opacity-100 focus-within:opacity-100">{workspace.role === "owner" && <MemberActionMenu workspace={workspace} member={member} reload={reload} />}</span></td></tr> })}</tbody></table></Card>
    {workspace.role === "owner" && invitations.length > 0 && <div className="mt-5"><h3 className="mb-2 text-sm font-semibold">Lời mời</h3><Card className="overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-4 py-3 font-medium">Email</th><th className="px-4 py-3 font-medium">Vai trò</th><th className="px-4 py-3 font-medium">Trạng thái</th><th className="px-4 py-3 font-medium">Hết hạn</th><th className="px-4 py-3 font-medium">Cập nhật</th></tr></thead><tbody>{invitations.map((invitation: WorkspaceInvitation) => <tr key={invitation.id} className="border-t"><td className="px-4 py-3 font-medium">{invitation.email}</td><td className="px-4 py-3 capitalize">{invitation.role}</td><td className="px-4 py-3"><span className={`rounded-md px-2 py-0.5 text-xs font-medium ${invitation.status === "ACCEPTED" ? "bg-emerald-100 text-emerald-700" : invitation.status === "PENDING" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}`}>{invitation.status}</span></td><td className="px-4 py-3 text-slate-600">{dateLabel(invitation.expiresAt)}</td><td className="px-4 py-3 text-slate-600">{dateLabel(invitation.updatedAt)}</td></tr>)}</tbody></table></Card></div>}
  </div>
}
